...
namespace Backupper
{
    class RESTBackups
    {
        ...
        public event EventHandler<RESTBackupsEA> GetBackupsComplete;
        ...
        async public Task<string> GetBackupsAsync(int timeoutRequestMs = 9000)
        {
            ...
            if (taskIndex == 0)
            {
                ...
                if (task.Result.StatusCode == System.Net.HttpStatusCode.OK)
                {
                    ...
                    GetBackupsComplete?.Invoke(this, new RESTBackupsEA(backupsSerialization));
                }
                return backupsSerialization;
            }
            else
            {
                throw new Exception("GetAsync has gone in timeout");
            } //With TaskCanceledException I can not read the exception message in the parent catch
        ...
        async public Task PutBackupsAsync(string backupsSerialization, int timeoutRequestMs = 9000)
        {
            ...
            if (taskIndex == 0)
            {
                ...
                {
                    Log.main.Add(new Mess(LogType.Warn, "", "ex.tostring:<" + ex.ToString() + ">"));
